...
Once enabled and merged, each push to `main` runs Node.js 24 with `npm ci`, then a blocking dependency audit (`npm audit --audit-level=info --include=dev`), then `npm test`, `npm run check`, and `npm run build`, then deploys `dist/` through the official GitHub Pages actions. Any reported vulnerability, including in dev or build-time dependencies, blocks a new deploy; audit service failures block new deploys too, but they do not affect the last published site. You can also run **Deploy GitHub Pages** manually from the Actions tab. The deploy job uses the `github-pages` environment and reports the deployment URL.
